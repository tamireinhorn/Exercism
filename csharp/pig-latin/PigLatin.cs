...
public static class PigLatin
{
    ...
    public static string TranslateWord(this string word)
    {
        ...
        if (StartsWithVowel){ // Rule 1, matching for vowel sounds. 
            return word + suffix;
        }
        else if (new Regex("^[a-z]y$").IsMatch(word)){ // Rule 4, matching for two letter words with y at the end. 
            ...
            if (!NextVowel.Success){ //Words with only consonants should be treated like this.
                return word + suffix;
                }
            if (HasQu.Success){
